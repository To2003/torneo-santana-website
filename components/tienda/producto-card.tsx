'use client'

import { useState } from 'react'
import { ShoppingCart, ImageOff } from 'lucide-react'
import { useCart } from './cart-context'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog'
import type { Producto } from '@/lib/types'

function formatearPrecio(n: number) {
  return `$${n.toLocaleString('es-AR')}`
}

export function ProductoCard({ producto }: { producto: Producto }) {
  const { agregarItem } = useCart()
  const [imagenError, setImagenError] = useState(false)
  const [agregado, setAgregado] = useState(false)
  const [modalAbierto, setModalAbierto] = useState(false)
  const [talle, setTalle] = useState<string | undefined>(undefined)
  const [color, setColor] = useState<string | undefined>(undefined)

  const necesitaTalle = producto.talles.length > 0
  const necesitaColor = producto.colores.length > 0
  const necesitaOpciones = necesitaTalle || necesitaColor
  const puedeConfirmar = (!necesitaTalle || talle) && (!necesitaColor || color)

  const confirmarAgregado = () => {
    setAgregado(true)
    setTimeout(() => setAgregado(false), 1500)
  }

  const agregarSinOpciones = () => {
    agregarItem({
      id: producto.id,
      productoId: producto.id,
      nombre: producto.nombre,
      precio: producto.precio,
      imagen: producto.imagen
    })
    confirmarAgregado()
  }

  const handleAgregarClick = () => {
    if (necesitaOpciones) {
      setTalle(undefined)
      setColor(undefined)
      setModalAbierto(true)
      return
    }
    agregarSinOpciones()
  }

  const handleConfirmarModal = () => {
    if (!puedeConfirmar) return

    const idLinea = [producto.id, talle, color].filter(Boolean).join('-')
    agregarItem({
      id: idLinea,
      productoId: producto.id,
      nombre: producto.nombre,
      precio: producto.precio,
      imagen: producto.imagen,
      talle,
      color
    })

    setModalAbierto(false)
    confirmarAgregado()
  }

  return (
    <>
      <div className="flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-md transition-shadow hover:shadow-lg">
        <div className="relative flex aspect-square items-center justify-center bg-slate-100">
          {producto.imagen && !imagenError ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={producto.imagen}
              alt={producto.nombre}
              className="h-full w-full object-cover"
              referrerPolicy="no-referrer"
              onError={() => setImagenError(true)}
            />
          ) : (
            <ImageOff className="h-10 w-10 text-muted-foreground/40" />
          )}
          {producto.tipo && (
            <span className="absolute left-2 top-2 rounded-full bg-torneo-primary px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-white shadow-sm">
              {producto.tipo}
            </span>
          )}
        </div>

        <div className="flex flex-1 flex-col gap-2 p-4">
          <h3 className="font-bold uppercase tracking-wide text-foreground">{producto.nombre}</h3>
          {producto.descripcion && (
            <p className="text-sm text-muted-foreground">{producto.descripcion}</p>
          )}
          <span className="text-lg font-black text-torneo-primary">{formatearPrecio(producto.precio)}</span>

          <button
            type="button"
            onClick={handleAgregarClick}
            className="mt-auto flex items-center justify-center gap-2 rounded-lg bg-torneo-primary py-2 text-sm font-semibold text-white transition-colors hover:bg-torneo-primary-dark"
          >
            <ShoppingCart className="h-4 w-4" />
            {agregado ? '¡Agregado!' : 'Agregar al carrito'}
          </button>
        </div>
      </div>

      {/* Popup para elegir talle/color antes de agregar */}
      <Dialog open={modalAbierto} onOpenChange={setModalAbierto}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{producto.nombre}</DialogTitle>
          </DialogHeader>

          <div className="space-y-3">
            {necesitaTalle && (
              <div>
                <label className="mb-1 block text-sm font-medium text-foreground">Talle</label>
                <Select value={talle} onValueChange={setTalle}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="Elegí un talle" />
                  </SelectTrigger>
                  <SelectContent>
                    {producto.talles.map((t) => (
                      <SelectItem key={t} value={t}>{t}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            )}

            {necesitaColor && (
              <div>
                <label className="mb-1 block text-sm font-medium text-foreground">Color</label>
                <Select value={color} onValueChange={setColor}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="Elegí un color" />
                  </SelectTrigger>
                  <SelectContent>
                    {producto.colores.map((c) => (
                      <SelectItem key={c} value={c}>{c}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            )}
          </div>

          <button
            type="button"
            onClick={handleConfirmarModal}
            disabled={!puedeConfirmar}
            className="flex items-center justify-center gap-2 rounded-lg bg-torneo-primary py-2.5 text-sm font-semibold text-white transition-colors hover:bg-torneo-primary-dark disabled:cursor-not-allowed disabled:opacity-50"
          >
            <ShoppingCart className="h-4 w-4" />
            Agregar al carrito
          </button>
        </DialogContent>
      </Dialog>
    </>
  )
}
